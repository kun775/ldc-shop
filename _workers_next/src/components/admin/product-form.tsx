    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
                e.preventDefault()
                if (formRef.current && !loading && !submitLock.current) {
    ...
    return (
        <AdminPageShell className="mx-auto max-w-7xl space-y-5 p-0.5">
            ...
            <div className="sticky top-0 z-20 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/70 bg-background/90 px-4 py-2.5 shadow-xs backdrop-blur-md">
                <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                        <span className="truncate text-sm font-semibold tracking-tight text-foreground">
                            {currentProduct?.name || (product ? t('admin.productForm.editTitle') : t('admin.productForm.addTitle'))}
                        </span>
                        {isManual ? (
                ...
                <div className="flex items-center gap-2">
                    <span className="hidden text-xs text-muted-foreground sm:inline-block">
                        <kbd className="rounded border border-border/80 bg-muted/50 px-1.5 py-0.5 text-[10px] font-medium">
                            {isMac ? '⌘S' : 'Ctrl+S'}
                        </kbd>{' '}
                    ...
                    {currentProduct?.id && (
                        <Button
                            ...
                            asChild
                        >
                            <Link href={`/buy/${currentProduct.id}`} target="_blank" rel="noreferrer">
                    ...
                    >
                        ...
                        <span>{loading ? t('admin.productForm.saving') : t('admin.productForm.saveButton')}</span>
                    </Button>
                </div>
            </div>
            ...
            <div className="relative rounded-2xl">
                ...
                <form ref={formRef} key={formSeed} onSubmit={handleSubmit} className="relative" aria-busy={loading}>
