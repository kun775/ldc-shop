...
*   **文件存储**: **Cloudflare R2** - 用于手动发货附件；未绑定 R2 时可降级到 D1 小文件存储。
...
- **订单与发货**:
    - ✅ **支付回调验签**: 签名与金额校验。
    - 🎁 **自动发货卡密**: 支付成功后自动发放卡密，缺货则标记已支付待处理；支持按商品配置发货备注，并在邮件和订单详情中显示于卡密之后。
    ...
    - 📎 **手动发货**: 支持发货说明和附件；R2 模式单文件最大 20MB，D1 降级模式单文件最大 1MB，每个订单最多 10 个附件。
...
- **管理后台**:
    ...
    - 🗂️ **卡密管理**: 批量导入、批量删除未使用卡密，并可为自动发货商品配置发货备注；多规格商品的每个规格需在对应商品下分别管理卡密。
    - 💳 **订单管理**: 分页/搜索/筛选、订单详情、标记已支付/已发货/取消；订单列表显示商品规格，手动发货支持说明与附件。
    - 🧹 **订单清理**: 支持批量选择与批量删除。
    - ⭐ **评价管理**: 搜索与删除评价。
    - 📦 **数据管理**: 全量导出 SQL（兼容 D1），支持从 Vercel 版 SQL 导入。
    - 📣 **公告管理**: 首页公告配置。
    - 👥 **顾客管理**: 查看顾客列表、积分管理、拉黑/解封；用户 ID 可跳转 Linux DO/GitHub，用户名和昵称进入站内顾客详情。
    - 🔄 **退款审核**: 退款申请列表可直接弹出关联订单详情，辅助核对支付、商品、顾客和履约信息。
    - 🗄️ **数据库升级**: 后台手动执行结构检查和幂等升级，支持失败记录、错误 ID 和重试，不在普通页面请求中自动执行 DDL。
    - 🧾 **审计与错误中心**: 查询关键用户/管理员操作及平台运行错误，支持服务端筛选、分页、脱敏详情、处理说明、标记已处理和重新打开。
    ...
    - 🔔 **更新检查**: 管理后台自动检测新版本并提示。
- **稳定性与加载体验**:
    - ⏳ **页面 Loading**: 路由和服务端数据加载超过短暂阈值后显示统一遮罩，快速响应不闪烁，异常时可恢复。
    - 🛡️ **提交保护**: 优惠券编辑、积分调整和手动发货等关键操作防止重复提交，并在成功、业务失败或网络异常后正确释放状态。
    - 🔎 **错误 ID**: 未知运行异常关联可查询错误 ID，客户端只展示安全信息，后台保留脱敏错误详情。
- **积分系统**:
...
项目默认通过 `FILES` 绑定将手动发货附件保存到名为 **`ldc-shop-files`** 的 R2 Bucket。部署前请在 Cloudflare Dashboard 的 **Storage & Databases → R2** 中创建该存储桶，或使用 Wrangler：

```bash
npx wrangler r2 bucket create ldc-shop-files
```

仓库中的 `wrangler.json` 已包含以下绑定：

```json
"r2_buckets": [
  {
    "binding": "FILES",
    "bucket_name": "ldc-shop-files"
  }
]
```

如果不使用 R2，可删除 `wrangler.json` 中的 `r2_buckets` 配置；代码会将附件降级保存到 D1，但单文件限制为 1MB。R2 模式单文件限制为 20MB，两种模式均限制每个订单最多 10 个附件。
...
部署完成后访问 Workers 域名并使用管理员账号登录。数据库结构升级不会在普通页面访问时自动执行；请进入 **管理后台 → 数据库升级**，检查待升级项并由管理员显式执行。升级过程支持幂等重试、结构复检和错误 ID 记录。
